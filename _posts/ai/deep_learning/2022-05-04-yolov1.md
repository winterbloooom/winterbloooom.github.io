...
{% include notice_box.html text="개인적인 공부를 위해 정리한 글입니다. 오류 및 오역과 의역, 생략이 많을 수 있습니다." %}
...
{% include inserted_box.html text="<b>PASCAL Visual Object Classes Challenge(VOC)</b> : <br>객체 탐지(object detection)와, 영상 분할(semantic segmentation), 분류(classification) 문제에서 벤치마크로 사용되는 데이터셋이다. PASCAL VOC 2012에는 20개의 카테고리가 있어 교통수단, 사람, 사물, 동물 등을 포함한다." %}
...
{% include inserted_box.html text="<b>Reduction layer</b> :<br>
...
{% include inserted_box.html text="<b>Sum of Squared Error(SSE)</b> : <br>
주로 회귀에서 사용하는 계산으로, 실제 데이터와 예측값의 차이를 제곱해 모두 더하는 것이다. $\mathrm{SSE} = \sum^{n}\limits_{i=1} (y_i - \hat{y_i})^2$로 나타낼 수 있다." %}
...
{% include inserted_box.html text="<b>상호 적응(co-adaptation)</b> : <br>
...
{% include inserted_box.html text="<u>비최대억제(NMS, Non-Maximal Suppression)</u> : <br>
...
{% include inserted_box.html text="<b>다운샘플링(Downsampling)</b> : <br>
